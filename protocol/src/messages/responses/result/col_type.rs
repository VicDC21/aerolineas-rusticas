...
pub enum ColType {
    ...
    ///
    /// Primero es precedido por un exponente ([Int](crate::aliases::types::Int)), y la base en formato [Varint](crate::messages::responses::result::col_type::ColType::Varint).
    Decimal,

    /// Un número de 8 bytes en formato IEEE 754 (Binary64) de precisión doble ([Double](crate::aliases::types::Double)).
    Double,

    /// Un número de 4 bytes en formato IEEE 754 (Binary32) de precisión simple ([Float](crate::aliases::types::Float)).
    Float,

    /// Un número de 4 bytes en complemento a dos ([Int](crate::aliases::types::Int)).
    Int,

    /// Número de 8 bytes en complemento a dos ([Long](crate::aliases::types::Long)) indicando el tiempo en milisegundos desde la
    /// _unix epoch_ (1ro de Enero de 1970, 00:00:00).
    ///
    /// Valores negativos indican una diferencia negativa a esa época.
    Timestamp,

    /// Número de 16 bytes (asumimos ([Uuid](crate::aliases::types::Uuid))) representando cualquier versión de un UUID.
    Uuid,
    ...
    /// Número de 16 bytes (asumimos [Uuid](crate::aliases::types::Uuid)) representando un UUID (Versión 1) tal y como está especificado en RFC 4122.
    Timeuuid,
    ...
    /// Numero de 8 bytes en complemento a dos ([Long](crate::aliases::types::Long)) que representa nanosegundos desde la medianoche.
    ...
    /// en este [Enum](crate::messages::responses::result::col_type::ColType).
    Map((Box<Self>, Box<Self>)),

    /// Un set con un tipo de columna especificado en este [Enum](crate::messages::responses::result::col_type::ColType).
    Set(Box<Self>),

    /// El valor tiene la forma `<n><type_1>...<type_n>` donde:
    ///
    /// * `<n>` es un número de 2 bytes ([Short]) representando el número de elementos.
    /// * `<type_i>` es el [tipo](crate::messages::responses::result::col_type::ColType) del i-ésimo valor de la tupla.
    Tuple(Vec<Box<Self>>),
}

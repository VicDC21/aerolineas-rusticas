...
pub enum Opcode {
    ...
    ///
    /// En cuyo caso, el servidor responderá con [READY](crate::headers::opcode::Opcode::Ready)
    /// o [AUTHENTICATE](crate::headers::opcode::Opcode::Authenticate).
    Startup,
    ...
    /// tipo de mensaje.
    ///
    /// Consiste en una serie de [AUTH_CHALLENGE](crate::headers::opcode::Opcode::AuthChallenge)S,
    /// seguidos de [AUTH_RESPONSE](crate::headers::opcode::Opcode::AuthResponse)S. Los detalles
    /// dependen del autenticador específico en uso.
    ///
    /// El intercambio termina con un [AUTH_SUCCESS](crate::headers::opcode::Opcode::AuthSuccess)
    /// desde el servidor, o un [ERROR](crate::headers::opcode::Opcode::RequestError).
    ///
    /// Este mensaje se manda como respuesta a una _Request_ [STARTUP](crate::headers::opcode::Opcode::Startup),
    /// pero autenticación es requerida, en cuyo caso el cliente ha de responder con [AUTH_RESPONSE](crate::headers::opcode::Opcode::AuthResponse).
    Authenticate,

    /// Indica una _Request_ en la que el cliente le pregunta al servidor qué tipo de opciones
    /// hay para un [STARTUP](crate::headers::opcode::Opcode::Startup).
    ///
    /// El servidor responderá con una _Response_ de tipo [SUPPORTED](crate::headers::opcode::Opcode::Supported).
    Options,

    /// Indica una _Response_ en consecuencia a una _Request_ [OPTIONS](crate::headers::opcode::Opcode::Options)
    /// del cliente, e indica las opciones disponibles para un potencial [STARTUP](crate::headers::opcode::Opcode::Startup).
    ///
    /// Dichas opciones y sus valores podrán ser expresados con un [multimap](crate::aliases::types::SupportedMultiMap)
    /// correspondiente.
    Supported,
    ...
    ///     - `<consistency>` es un dato de tipo [Consistency](crate::notations::consistency::Consistency).
    ///     - `<flags>` es un [Int](crate::aliases::types::Int) donde sus bits representan
    ///       datos de tipo [QueryFlag](crate::messages::requests::query_flags::QueryFlag).
    ///
    /// <div class="warning">
    ///
    /// El parámetro de [Consistency](crate::notations::consistency::Consistency) podría
    /// ser ignorado según cada _query_ necesite.
    ///
    /// </div>
    ///
    /// En un caso exitoso, el servidor responderá con una _Response_ de tipo [RESULT](crate::headers::opcode::Opcode::Result).
    Query,

    /// Indica una _Response_ a una _query_ (esto es, a un mensaje de tipo [QUERY](crate::headers::opcode::Opcode::Query),
    /// [PREPARE](crate::headers::opcode::Opcode::Prepare), [EXECUTE](crate::headers::opcode::Opcode::Execute)
    /// o [BATCH](crate::headers::opcode::Opcode::Batch)).
    ///
    /// El contenido del mensaje vendrá acompañado de un [Int](crate::aliases::types::Int)
    /// indicando el [tipo](crate::messages::responses::result_kinds::ResultKind) de resultado.
    ...
    /// [ejecución](crate::headers::opcode::Opcode::Execute).
    ///
    /// El contenido tendrá el formato `<query><flags>[<keyspace>]`, donde:
    /// * `<query>` es un [String] representando la _query_ misma.
    /// * `<flags>` es un [Int](crate::aliases::types::Int) cuyos bits corresponden a datos
    ///   de tipo [PrepareFlag](crate::messages::requests::query_flags::QueryFlag).
    ///
    /// En caso de éxito, el servidor responderá con un mensaje [RESULT](crate::headers::opcode::Opcode::Result)
    /// de tipo [Prepared](crate::messages::responses::result_kinds::ResultKind::Prepared).
    Prepare,

    /// Indica una _request_ en la que el cliente pide ejecutar una _query_ [preparada](crate::headers::opcode::Opcode::Prepare).
    ///
    ...
    ///   Debería ser el mismo ID que el devuelto por una respuesta a una _Request_ [PREPARE](crate::headers::opcode::Opcode::Prepare).
    /// * `<result_metadata_id>` es un set de metadatos de cambios aplicados, enviados en conjunto
    ///   en una _Response_ [Result](crate::headers::opcode::Opcode::Result)/[Rows](crate::messages::responses::result_kinds::ResultKind::Rows).
    /// * `<query_parameters>` tiene la misma definición que [QUERY](crate::headers::opcode::Opcode::Query).
    Execute,

    /// Indica una _Request_ para registrar esta conexión para "escuchar" [eventos](crate::headers::opcode::Opcode::Event).
    ///
    /// El servidor responderá con una _Response_ [READY](crate::headers::opcode::Opcode::Ready).
    ///
    /// <div class="warning">
    ///
    ...
    /// Indica una _Response_ en la que el server comunica un [evento](crate::messages::responses::events::event_types::EventType).
    ///
    /// Un cliente sólo escuchará eventos a los que se ha [registrado](crate::headers::opcode::Opcode::Register).
    ///
    ...
    /// ([preparadas](crate::headers::opcode::Opcode::Prepare) o no).
    ///
    ...
    /// * `<type>` será un [BatchType](crate::messages::requests::batch_types::BatchType).
    /// * `<flags>` es un [Int](crate::aliases::types::Int) cuyos bits corresponden a datos
    ///   de tipo [BatchFlag](crate::messages::requests::batch_flags::BatchFlag).
    ///   Estas flags son similares a los de [QUERY](crate::headers::opcode::Opcode::Query)
    ///   y [EXECUTE](crate::headers::opcode::Opcode::Register), excepto que los últimos 4 bits
    ///   son siempre `0` pues sus opciones no tienen sentido en el contexto de un _batch_.
    /// * `<n>` es un [Short](crate::aliases::types::Short) indicando la cantidad de _queries_ a continuación.
    ...
    ///     - `<n>` es un [Short](crate::aliases::types::Short) indicando la cantidad de valores
    ///       a continuación _(podría ser `0`)_.
    ///     - `<name_i>` es un nombre opcional al valor que sigue. Sólo ha de estar presente si se
    ///       declaró la [flag correspondiente](crate::messages::requests::batch_flags::BatchFlag::WithNamesForValues).
    ///     - `<value_i>` es un [Value](crate::notations::value::Value) para la variable `i`.
    Batch,
    ...
    /// Indica una respuesta a un [desafío](crate::headers::opcode::Opcode::AuthChallenge)
    /// de autenticación por parte del servidor.
    ///
    /// El contenido es un conjunto de [Byte]s cuyo significado cambia según el autenticador en uso.
    ///
    /// El servidor responderá con un mensaje [AUTH_CHALLENGE](crate::headers::opcode::Opcode::AuthChallenge)
    /// para seguir con la autenticación, [AUTH_SUCCESS](crate::headers::opcode::Opcode::AuthSuccess) si
    /// la autenticación terminó con éxito, o [ERROR](crate::headers::opcode::Opcode::RequestError) si
    ...
    fn try_from(byte: Byte) -> Result<Self> {
        match byte {
            ...
            _ => Err(Error::ConfigError(format!(
                "El opcode recibido no es valido: {byte}"
            ))),
        }
    }
}

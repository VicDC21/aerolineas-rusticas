...
impl TryFrom<Vec<u8>> for QueryFlag {
    ...
    fn try_from(int: Vec<u8>) -> Result<Self, Self::Error> {
        let bytes_array: [u8; 4] = match int.try_into(){
            ...
            Err(_e) => return Err(Error::ConfigError("No se pudo castear el vector de bytes en un array en QueryFlag".to_string()))
        };
        ...
        match value {
            ...
            _ => Err(Error::ConfigError("La flag indicada para query no existe".to_string())),         
        }
    }
}

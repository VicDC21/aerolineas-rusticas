...
use std::{iter::Map, net::IpAddr};

use crate::cassandra::notations::consistency::Consistency;
...
pub enum Error {
    ...
    /// * `<blockfor>` es un número ([u32]) que representa la cantidad de réplicas cuya confirmación es necesaria para cumplir `<cl>`.
    ...
    /// * `<cl>` es el nivel de [Consistency](crate::cassandra::notations::consistency::Consistency) de la query que lanzó esta excepción.
    /// * `<received>` es un número ([u32]) que representa la cantidad de nodos que han respondido a la request.
    /// * `<blockfor>` es un número ([u32]) que representa la cantidad de réplicas cuya respuesta es necesaria para cumplir `<cl>`.
    /// * `<reasonmap>` es un "mapa" de endpoints a códigos de razón de error. Esto mapea los endpoints de los nodos réplica que fallaron al ejecutar la request a un código representando la razón del error. La forma del mapa es empezando con un [u32] n seguido por n pares de <endpoint><failurecode> donde <endpoint> es un [IpAddr](std::net::IpAddr) y <failurecode> es un [u8].
    /// * `<data_present>` es un [u8] (representa un booleano: 0 es false, distinto de 0 es true) que indica si el nodo al que se le hizo el pedido de la data respondió o no.
    ...
    /// * `<received>` es un número ([u32]) que representa la cantidad de nodos que han reconocido la request.
    /// * `<blockfor>` es un número ([u32]) que representa la cantidad de réplicas cuya confirmación es necesaria para cumplir `<cl>`.
    CASWriteUnknown(String, Consistency, u32, u32),

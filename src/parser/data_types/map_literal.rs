use crate::{cassandra::errors::error::Error, parser::statements::{ddl_statement::ddl_statement_parser::check_words, dml_statement::dml_statement_parser::is_term}};
        ...
        if check_words(lista, "{"){
            while check_words(lista, ",") || !check_words(lista, "}"){
                let term = match is_term(lista)?{
                    Some(value) => value,
                    None => return Err(Error::SyntaxError("Sintaxis de mapa incorrecta".to_string()))
                };
                if !check_words(lista, ":"){
                    return Err(Error::SyntaxError("Sintaxis de mapa incorrecta".to_string()));
                }

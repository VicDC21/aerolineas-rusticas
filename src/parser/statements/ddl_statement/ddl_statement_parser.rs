use crate::{
    ...
    parser::data_types::{
        identifier::Identifier, keyspace_name::KeyspaceName, option::Options, unquoted_name::UnquotedName
    },
};

use super::{alter_keyspace::AlterKeyspace, alter_table::AlterTable, create_keyspace::CreateKeyspace,
 create_table::CreateTable, drop_keyspace::DropKeyspace, drop_table::DropTable, truncate::Truncate};
...
pub fn is_an_option(lista: &mut Vec<String>) -> Result<Option<Options>, Error>{
    let value = match Identifier::check_identifier(lista)?{
        Some(value) => value,
        None => return Err(Error::SyntaxError("Debe haber un valor luego del WITH".to_string()))
     };
    ...
    if !check_words(lista, "="){
        return Err(Error::SyntaxError("Falto el '=' de las opciones".to_string()));
    }

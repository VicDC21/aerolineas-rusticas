...
use super::{data_types::{identifier::identifier::Identifier, literal::list_literal::ListLiteral, term::Term}, statements::ddl_statement::ddl_statement_parser::check_words};
...
impl Assignment{
    ...
    pub fn check_kind_of_assignment(lista: &mut Vec<String>)-> Result<Option<Assignment>, Error>{
        ...
        if let Some(values) = Assignment::check_column_name_col_term(lista)?{
            return Ok(Some(Assignment::ColumnNameColTerm(column_name, values.0, values.1)))
        }
        if let Some(values) = Assignment::check_column_name_list_col(lista)?{
            return Ok(Some(Assignment::ColumnNameListCol(column_name, values.0, values.1)))
        };
        Err(Error::SyntaxError("Tipo de dato incorrecto al hacer SET".to_string()))
    }
    ...
    fn check_column_name_col_term(lista: &mut Vec<String>) -> Result<Option<(Identifier, Term)>, Error>{
        let column_name = match Identifier::check_identifier(lista)?{
            Some(value) => value,
            None => return Ok(None)
        };
    ...
    fn check_column_name_list_col(lista: &mut Vec<String>) -> Result<Option<(ListLiteral, Identifier)>, Error>{

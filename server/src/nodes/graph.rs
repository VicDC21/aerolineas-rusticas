...
/// La cantidad de nodos que comenzarán su intercambio de _gossip_ con otros [n](crate::nodes::graph::HANDSHAKE_NEIGHBOURS) nodos.
...
impl NodesGraph {
    ...
    fn bootup_new_nodes(&mut self, n: Byte) -> Result<Vec<Option<NodeHandle>>> {
        ...
        for _i in 0..n {
            ...
            let _current_id = self.add_node_id();
    ...
    fn bootup_existing_nodes(&mut self, n: Byte) -> Result<Vec<Option<NodeHandle>>> {
        ...
        for i in 0..n {
            ...
            if path.exists() {
                let node: Node = load_json(&node_path)?;
